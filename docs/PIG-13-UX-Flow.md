...
| **G3** | 引導文案**偏向「請往後退」** | 誤差敏感度不對稱：同樣偏 0.5 m，**太近的角點偏差是太遠的 1.9 倍**；偏 1.0 m 時是 **3.8 倍**。更關鍵的是失效方式不同 —— 退太遠只是車體變小（可回復），站太近會讓**全車直接出框**（低於約 3.2 m 就拍不完整）。見下方敏感度表。 |
...
> **距離的量法**：表中距離是相機到**車體中心地面投影點**的水平距離（與 `build-report.json` 一致）。
> 基準 3.85 m 時相機其實離最近的車體外框角只有約 **1.9 m** —— 拿捲尺量的人請量到車體中心，
> 不要量到保桿，兩者差快兩公尺。同一組計算下，低於約 **3.2 m** 全車就塞不進 4:3 畫面，
> 超過約 **5.0 m** 車體只剩不到 60% 畫面寬、細節解析不足；3.7–3.9 m 落在中間（車體約占 79–84% 寬）。

**誤差敏感度（G3）**

以 **Toyota Corolla Cross XG10**（4460 × 1825 × 1620 mm、軸距 2640 mm）的車體外接框，
用基準相機（**3.85 m / 1.5 m / 45° / 26 mm、hFOV 69.39°、4:3 橫向**）投影出基準輪廓，
再**單次只改一個參數**重投影，量外接框八個角點的最大位移。
讀數 = 最大位移 ÷ 畫面上的車寬，四角（LF/RF/LR/RR）取最差值。
相機模型即 [`demo/guide-lab/geom.js`](../demo/guide-lab/geom.js) 的 `makeCam`，注視點取自
[`build-report.json`](../demo/assets/car-reference/generated/build-report.json)。
自我檢查：基準值代入得 0.0%；相機高與焦距的正負擾動對稱（比值 1.00–1.01×）。

| 偏離基準（單次只改一項） | 角點偏差 | 車體占畫面寬 |
|----------|:--------:|:---:|
| 距離 2.85 m（**−1.0 m**） | 🔴 **66%** | 134% → **出框** |
| 距離 3.35 m（**−0.5 m**） | 🟠 19% | 94%（貼邊） |
| 距離 4.35 m（+0.5 m） | 🟢 11% | 70% |
| 距離 4.85 m（+1.0 m） | 🟡 18% | 62% |
| 偏擺 30°（**−15°**） | 🟠 27% | 75% |
| 偏擺 35°（−10°） | 🟡 17% | 77% |
| 偏擺 55°（+10°） | 🟢 11% | 82% |
| 偏擺 60°（+15°） | 🟡 15% | 88% |
| 相機高 1.3 / 1.7 m（±0.2 m） | 🟢 6.0% | 80% |
| 相機高 1.1 / 1.9 m（±0.4 m） | 🟡 12% | 79% |
| 等效焦距 24 mm | 🟢 4.4% | 74% |
| 等效焦距 28 mm | 🟢 4.4% | 86% |
| *（基準 3.85 m / 1.5 m / 45° / 26 mm）* | *0.0%* | *80%* |

三個結論：

1. **太近確實比太遠貴，但不是八倍。** 同幅度偏移下近／遠比為 **1.9×（±0.5 m）** 與 **3.8×（±1.0 m）**。
   真正該講的是**失效方式**：退太遠只是車體變小、輪廓仍可判讀；站太近會讓全車出框，
   低於約 3.2 m 就再也拍不到完整車身。所以引導文案仍一律偏向**叫人往後退**。
2. **手機鏡頭差異幾乎不必管**（24/28 mm 只有 4.4%，全表最小）。就算是最溫和的站位誤差（+0.5 m，11%）
   都比它大 2.4 倍，−0.5 m 大 4.4 倍、−1.0 m 大 15 倍。**站位遠比器材重要**。
3. **偏擺角與距離同等重要**（舊表沒有涵蓋）：偏擺 ±10° 的 11–17% 與距離 ∓0.5 m 的 11–19% 是同一量級，
   且同樣不對稱（轉向車頭方向的 −15° 為 27%，是本表除「站太近」外最大的一項）。
   這也是 [`guide-lab.html`](../demo/guide-lab.html) 把偏擺標為近似重投影最弱一軸的原因。

<sub>此表為 2026-08 用定案基準（Corolla Cross + 3.85 m）重算的結果，取代先前以轎車 3.3 m 為基準的舊表。
舊表的「站太近 2.5 m → 85%」「相機高 1.3 vs 1.5 m → 11%」「鏡頭 24 vs 26 mm → 8%」數字**已作廢**：
基準距離不同，量級不可直接沿用（同一顆車在新基準下，2.5 m 對應的偏差是 **175%**、3.8 m 只有 **1.4%**）。</sub>

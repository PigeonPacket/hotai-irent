...
橫向路徑：同一張 `<img 1200×1600>` → cover 進 1440×1080（上下各裁掉天空／地面，全車仍在畫面內）→
取樣 160×120。

> ⚠️ 下表**直向**欄位的取樣路徑含「底部 150 px `rgba(0,0,0,0.45)` 標籤條」，
> 但現行 `drawAssetFrame()` 畫的是**左下角小標籤片**（`24*S` 起、`56*S` 高、`rgba(0,0,0,0.55)`），
> 不是整條橫幅。重驗時兩個方向請一起重跑，不要只補一半。
...
|---|---|---|---|---|
| `car-lf.jpg` | 145.22 | 33.21 | 13.648 | 0.1689 |
| `car-rf.jpg` | 132.60 | 45.85 | 14.090 | 0.0944 |
| `car-lr.jpg` | 134.84 | 41.08 | 14.085 | 0.1175 |
| `car-rr.jpg` | 126.49 | 46.64 | 14.666 | 0.0989 |
| `car-default.jpg` | 150.87 | 29.99 | 11.824 | 0.1554 |
...
>
> **橫向下五張的 `gradient` 全部落在 11.8–14.7，`gradient ≥ 9` 這道保險絲一律觸發，
> 模糊檢查根本不會執行。** `sharpness`（0.094–0.169）雖然也全部過關，但在橫向下
> **不是**判定依據，只是餘裕證明。直向下 `gradient` 只有 9.44–11.32，
> 其中 `car-default.jpg` 的 9.44 離門檻 9 只剩 5% —— 直向才是比較緊的那一邊。
>
> 方向換成橫向後所有指標都變好（`gradient` 9.44–11.32 → 11.82–14.67），
> 因為 cover 進橫向畫布時上下的平坦天空／地面被裁掉，車體占比提高、高頻內容比例上升。
>
> 沒有步驟 4 的 clarity 時，直向實測 `gradient` 只有 6.8–8.0、`sharpness` 0.030–0.040
...
（車身占比更高，指標反而更好 —— 逐檔數字見上方「橫向 1440×1080」表）。
